    ...
    #             continue
        ...
        with open(file_path, "r") as f:
            ...
            for line in f:
                ...
                class_id = str(int(float(splitted_line[5])))
                if bb_type == BBType.GROUND_TRUTH:
                    confidence = None
                    x1 = float(splitted_line[0])
                    y1 = float(splitted_line[1])
                    w = float(splitted_line[2])
                    h = float(splitted_line[3])
                elif bb_type == BBType.DETECTED:
                    confidence = float(splitted_line[4])
                    x1 = int(float(splitted_line[0]))
                    y1 = int(float(splitted_line[1]))
                    w = int(float(splitted_line[2]))
                    h = int(float(splitted_line[3]))
